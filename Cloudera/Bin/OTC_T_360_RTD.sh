...
###########################################################################################################################################################
echo `date '+%Y-%m-%d %H:%M:%S'`" INFO: Parametros definidos en la tabla params_des" >> $VAL_LOG_EJECUCION
###########################################################################################################################################################
...
TDSERVICE=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'TDSERVICE';"`
TDCLASS_ORC=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$SPARK_GENERICO"' AND parametro = 'TDCLASS_ORC';"`          
RUTA_PYTHON=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'RUTA_PYTHON';"` 
RUTA_LIB=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'RUTA_LIB';"` 
RUTA_LIB_ORACLE=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'RUTA_LIB_ORACLE';"` 
VAL_ESQUEMA_TMP=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ESQUEMA_TMP';"`
TABLA_PIVOTANTE=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'TABLA_PIVOTANTE';"` 
COMBO_DEFECTO=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'COMBO_DEFECTO';"`
...
VAL_ETP02_MASTER=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ETP02_MASTER';"`
VAL_ETP02_DRIVER_MEMORY=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ETP02_DRIVER_MEMORY';"`
VAL_ETP02_EXECUTOR_MEMORY=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ETP02_EXECUTOR_MEMORY';"`
VAL_ETP02_NUM_EXECUTORS=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ETP02_NUM_EXECUTORS';"`
VAL_ETP02_NUM_EXECUTORS_CORES=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_ETP02_NUM_EXECUTORS_CORES';"`
###########################################################################################################################################################
...
###########################################################################################################################################################
echo `date '+%Y-%m-%d %H:%M:%S'`" INFO: Validando si son los 3 primer dias de inicion de mes " >> $VAL_LOG_EJECUCION
###########################################################################################################################################################
...
--vIPtMes=$MES_PROCESO \
--vSComboDefecto="$COMBO_DEFECTO" >> $VAL_LOG_EJECUCION
...
if [ "$ETAPA" = "2" ]; then
###########################################################################################################################################################
echo `date '+%Y-%m-%d %H:%M:%S'`" INFO: ETAPA 2: Exportar a Oracle " >> $VAL_LOG_EJECUCION
###########################################################################################################################################################
$VAL_RUTA_SPARK \
--name $ENTIDAD \
