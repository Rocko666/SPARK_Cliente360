...
@seguimiento_transformacion
def fun_sobreescribir_datos(sqlContext,df, bdd, nombre_tabla):
    try:
        val_base_tabla = '%s.%s' % (bdd.lower(), nombre_tabla.lower())
        numero = fun_validacion_tablas(sqlContext, bdd, nombre_tabla.lower())
        if numero > 0:
            df.repartition(10).write.mode("overwrite").insertInto(val_base_tabla, overwrite=True)
            print(msg_succ('\n|Info|Se Cargaron correctamente los datos Tabla: %s.%s \n' % (bdd, nombre_tabla.lower())))
        else:
            df.repartition(10).write.format('hive').saveAsTable(val_base_tabla)
            print(msg_succ('\n|Info|Se Cargaron correctamente los datos Tabla: %s.%s \n' % (bdd, nombre_tabla.lower())))
        return 0
    except Exception as e:
        print(msg_error('Error PySpark:\n %s ' % e))
...
@seguimiento_transformacion
def fun_sobreescribir_datos_pt(sqlContext,df, bdd, nombre_tabla, col_part):
    try:
        val_base_tabla = '%s.%s' % (bdd.lower(), nombre_tabla.lower())
        numero = fun_validacion_tablas(sqlContext, bdd, nombre_tabla.lower())
        if numero > 0:
            ...
            df.repartition(10).write.partitionBy(col_part).format('hive').saveAsTable(val_base_tabla)
            print(msg_succ('\n|Info|Se Cargaron correctamente los datos Tabla: %s.%s \n' % (bdd, nombre_tabla.lower())))
        return 0
    except Exception as e:
        print(msg_error('Error PySpark:\n %s ' % e))
