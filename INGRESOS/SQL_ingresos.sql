--- script desde ruta /RGenerator/reportes/Cliente360/sql
------------------------------------------------------------------------------------------------------------------
-- NOMBRE: carga_otc_t_360_ingresos_tmp.sql.sql
-- DESCRIPCION:
--   HQL que carga la informacion en la tablas temporales en Hive
-- AUTOR: Brigitte Balon - Softconsulting
-- FECHA CREACION: 2022-06-27
------------------------------------------------------------------------------------------------------------------
-- MODIFICACIONES
...
DROP TABLE IF EXISTS db_reportes.otc_t_360_parque_ingresos;
CREATE TABLE db_reportes.otc_t_360_parque_ingresos AS
SELECT t1.* FROM
...
----------------------------	
--(KV 303551) modificacion se agregan los campos t3.ingreso_recargas_30,t3.cantidad_recargas_30,t3.ingreso_bonos_30,
--t3.cantidad_bonos_30,t3.ingreso_combos_30,t3.cantidad_combos_30 para obtener PARQUE_RECARGADOR_30_DIAS
----------------------------
DROP TABLE IF EXISTS db_reportes.otc_t_360_ingresos_temp;
CREATE TABLE db_reportes.otc_t_360_ingresos_temp AS
SELECT 
t1.num_telefonico,
t1.numero_abonado,
t1.account_num,
...
(
COALESCE(t3.ingreso_recargas_m0,0) --RECARGAS DEL MES
+COALESCE(t3.ingreso_bonos,0)+COALESCE(t3.ingreso_combos,0) --BONOS Y COMBOS DEL MES EN PUNTO DE VENTA
+COALESCE(t4.valor_sms_periodo,0) --OD SMS
+COALESCE(t4.valor_voz_periodo,0) --OD VOZ
+COALESCE(t4.valor_datos_periodo,0) --OD DATOS
+COALESCE(t4.valor_contenido_periodo,0) --OD CONTENIDOS
+COALESCE(t4.valor_buzon_voz_periodo,0) --OD BUZON DE VOZ
+COALESCE(t4.valor_llamada_espera_periodo,0) --OD LLAMADA EN ESPERA
+COALESCE(t4.valor_adelanto_saldo_periodo,0) -- OD ADELANTO SALDO
) AS ingresos_prepago,
...
---------------------------
--CAMBIO ABRIL 2020 tabla t3
----------------------------
DROP TABLE IF EXISTS db_reportes.otc_t_360_ingresos_fact_pos_group;
CREATE TABLE db_reportes.otc_t_360_ingresos_fact_pos_group AS
...
DROP TABLE IF EXISTS db_reportes.otc_t_360_ingresos_fact_pos_group_1;
CREATE TABLE db_reportes.otc_t_360_ingresos_fact_pos_group_1 AS
SELECT t.abonado
,t.account_num
...
DROP TABLE IF EXISTS db_reportes.otc_t_360_ingresos_fact_pos;
CREATE TABLE db_reportes.otc_t_360_ingresos_fact_pos AS
SELECT t.abonado
,t.account_num
...
DROP TABLE IF EXISTS db_temporales.tmp_360_ingreso_od_datos_pos;
CREATE TABLE db_temporales.tmp_360_ingreso_od_datos_pos AS
...
DROP TABLE IF EXISTS db_temporales.tmp_360_ingreso_od_datos_pos_mes;
CREATE TABLE db_temporales.tmp_360_ingreso_od_datos_pos_mes AS
...
----------------------------	
--(KV 303551) modificacion se agregan los campos t.ingreso_recargas_30,t.cantidad_recargas_30,t.ingreso_bonos_30,
--t.cantidad_bonos_30,t.ingreso_combos_30,t.cantidad_combos_30 para obtener PARQUE_RECARGADOR_30_DIAS
----------------------------
DROP TABLE IF EXISTS db_reportes.otc_t_360_ingresos_temp_ok;
CREATE TABLE db_reportes.otc_t_360_ingresos_temp_ok AS
...
--then ROUND(t2.valor_ingreso_pre,2)
...
--LEFT OUTER JOIN db_reportes.otc_t_360_ingresos_fact_pre t2
--ON t2.numero_abonado=t.numero_abonado
--AND t2.account_num=t.account_num
...
----sql2
---EJECUCION de la DROP PARTITION en HIVE
set hive.cli.print.header=false;
		set hive.vectorized.execution.enabled=false;
		set hive.vectorized.execution.reduce.enabled=false;
	           ALTER TABLE db_reportes.otc_t_360_ingresos DROP IF EXISTS PARTITION(fecha_proceso=$FECHAEJE);



--carga_otc_t_360_ingresos.sql
------------------------------------------------------------------------------------------------------------------
-- NOMBRE: carga_otc_t_360_ingresos_tmp.sql
-- DESCRIPCION:
--   HQL que carga la informacion en la tabla destino en Hive
-- AUTOR: Brigitte Balon - Softconsulting
-- FECHA CREACION: 2022-06-27
------------------------------------------------------------------------------------------------------------------
-- MODIFICACIONES
...
----------------------------	
--(KV 303551) modificacion se agregan los campos t1.ingreso_recargas_30,t1.cantidad_recargas_30,t1.ingreso_bonos_30,
--t1.cantidad_bonos_30,t1.ingreso_combos_30,t1.cantidad_combos_30 para obtener PARQUE_RECARGADOR_30_DIAS
----------------------------
INSERT INTO db_reportes.otc_t_360_ingresos PARTITION(fecha_proceso)
