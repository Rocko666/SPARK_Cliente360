...
-- DROP TABLES
DROP TABLE IF EXISTS ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_new;
DROP TABLE IF EXISTS ${VAL_ESQUEMA_TMP}.tmp_saldos;
DROP TABLE IF EXISTS ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_categoria_ultimo_mes;
DROP TABLE IF EXISTS ${VAL_ESQUEMA_TMP}.${HIVETABLE}_prev;

CREATE TABLE ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_new AS  
...
--,COALESCE(a.fecha_movimiento_mes, gen.fecha_alta_segmento, gen.fecha_alta,CAST(from_unixtime(unix_timestamp(a.fecha_alta),'yyyy-MM-dd') AS date)) AS fecha_alta
,CAST(from_unixtime(unix_timestamp(a.fecha_alta),'yyyy-MM-dd') AS date) AS fecha_alta
--,CAST(months_between(CAST(date_format(from_unixtime(unix_timestamp(CAST(${FECHA_EJE_1} AS string), 'yyyyMMdd')), 'yyyy-MM-01') AS date), date_format(COALESCE(a.fecha_movimiento_mes, gen.fecha_alta_segmento, gen.fecha_alta,CAST(from_unixtime(unix_timestamp(a.fecha_alta),'yyyy-MM-dd') AS date)), 'yyyy-MM-01')) AS int)+ 1 AS meses
,CAST(months_between(CAST(date_format(from_unixtime(unix_timestamp(CAST(${FECHA_EJE_1} AS string), 'yyyyMMdd')), 'yyyy-MM-01') AS date), date_format(CAST(from_unixtime(unix_timestamp(a.fecha_alta),'yyyy-MM-dd') AS date), 'yyyy-MM-01')) AS int)+ 1 AS meses
,CASE WHEN grupo_prepago IN ('1', '3') THEN 'DATOS' WHEN grupo_prepago IN ('2', '4') THEN 'VOZ' ELSE 'DATOS' END preferencia_consumo
FROM ${TABLA_PIVOTANTE} a
...
LEFT JOIN db_reportes.otc_t_360_modelo m ON a.num_telefonico = m.num_telefonico AND m.fecha_proceso= ${FECHA_MENOS_1D} AND a.fecha_proceso = m.fecha_proceso
LEFT JOIN db_temporales.tmp_360_app_mi_movistar app ON (a.num_telefonico = app.num_telefonico)
LEFT JOIN (SELECT num_telefonico,grupo_prepago,tiene_score_tiaxa,bancarizado,ticket_recarga,limite_credito,tipo_movimiento_mes,portabilidad_movimiento_mes,fecha_movimiento_mes,fecha_alta_segmento,CAST(from_unixtime(unix_timestamp(fecha_alta),'yyyy-MM-dd') AS date) fecha_alta FROM db_reportes.otc_t_360_general WHERE fecha_proceso = ${FECHA_MENOS_1D} AND es_parque = 'SI') gen ON (a.num_telefonico = gen.num_telefonico)
WHERE mv.codigo_plan IS NULL
AND UPPER(a.sub_segmento) <> 'PREPAGO BLACK'
AND a.es_parque = 'SI'
AND upper(a.estado_abonado) = 'AAA';

CREATE TABLE ${VAL_ESQUEMA_TMP}.tmp_saldos AS  
SELECT 
B.saldo1,
B.m0_nurin,
B.p_fecha fecha_saldo
FROM  db_altamira.otc_t_saldos B 
WHERE B.p_fecha = ${FECHA_MENOS_1D}
AND saldo1 > 0;

--CAMBIAR EL ESQUEMA
CREATE TABLE ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_categoria_ultimo_mes AS
SELECT num_telefono             
,ticket_mes_ant           
,bono_combo               
,recargas                 
...
,categoria                
,duracion_oferta                             
FROM ${HIVEDB}.otc_t_rtd_categoria_ultimo_mes
WHERE pt_mes=${MES_PROCESO};

CREATE TABLE ${VAL_ESQUEMA_TMP}.${HIVETABLE}_prev AS
...
--,CASE WHEN UPPER(t2.categoria)='NO_RECURRENTE' THEN 'PARQUE_NO_RECURRENTE' ELSE t1.movimiento END AS movimiento
,t1.movimiento
...
FROM ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_new t1 
LEFT JOIN ${VAL_ESQUEMA_TMP}.tmp_otc_t_rtd_categoria_ultimo_mes t2 ON t1.numero_telefono =t2.num_telefono
LEFT JOIN ${HIVEDB}.otc_t_rtd_beneficios_promocionales t3
...
and t3.nivel<=1
LEFT JOIN ${VAL_ESQUEMA_TMP}.tmp_saldos t4 ON t4.m0_nurin = t1.numero_telefono
LEFT JOIN ${HIVEDB}.otc_t_rtd_matriz_oferta_rtd_sin_saldo t5 ON
upper(t1.movimiento) = upper(t5.segmento)
and t5.app = if(t1.app=1,'SI','NO')
and t5.bancarizado = if(t1.bancarizado=1,'SI','NO')
LEFT JOIN ${HIVEDB}.otc_t_rtd_matriz_oferta_rtd_sin_saldo t6 ON
UPPER(t2.categoria) = upper(t6.segmento)
and t6.app = if(t1.app=1,'SI','NO')
and t6.bancarizado = if(t1.bancarizado=1,'SI','NO');

ALTER TABLE ${HIVEDB}.${HIVETABLE} DROP IF EXISTS PARTITION (fecha_proceso=${VAL_FECHA_PROCESO});
INSERT INTO TABLE ${HIVEDB}.${HIVETABLE} PARTITION (fecha_proceso=${VAL_FECHA_PROCESO}) 
SELECT abonado_cd                
, numero_telefono           
, linea_negocio             
, app                       
...
, tipo_consumidor           
, movimiento                
, CASE WHEN tipo_consumidor IS NULL AND upper(movimiento) IN ('PARQUE') AND oferta_sugerida IS NULL AND flag_lista_blanca = 1 THEN '${COMBO_DEFECTO}' ELSE oferta_sugerida END AS oferta_sugerida         
, beneficio                 
, fecha_inicio_benef        
, fecha_fin_benef           
, duracion_oferta           
...
, duracion_beneficio        
, combo_complemento                     
FROM ${VAL_ESQUEMA_TMP}.${HIVETABLE}_prev;
