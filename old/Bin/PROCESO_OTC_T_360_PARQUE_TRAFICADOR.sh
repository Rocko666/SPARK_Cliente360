...
# $4: Nombre de la Carpeta del Proyecto                 	Reingenieria_Cliente360								#
# $5: Nombre del Proceso a Ejecutar                 		OTC_T_360_PARQUE_TRAFICADOR						        	#
...
echo "Parametros enviados a la shell:" $'\n(1)' $1 $'\n(2)' $2 
...
VAL_NOMBRE_PROYECTO=`mysql -N  <<<"select valor from params_des where ENTIDAD = '"$ENTIDAD"' and parametro = 'VAL_NOMBRE_PROYECTO';"`
...
if [ -z "$VAL_COLA_EJECUCION" ] || [ -z "$VAL_CADENA_JDBC" ] || [ -z "$VAL_USUARIO_BEELINE" ] ; then
    error=1
    echo " $TIME [ERROR] $rc alguno de los parametros de MySQL esta vacio o nulo"
    exit $error
fi
...
VAL_RUTA_LOG=$VAL_RUTA_PROCESO/$VAL_NOMBRE_PROYECTO/Log
...
f_check=`date -d "$VAL_FECHA_EJECUCION" "+%d"`

        if [ $f_check == "01" ];
        then
        f_inicio=`date -d "$VAL_FECHA_EJECUCION -1 days" "+%Y%m01"`
        else
        f_inicio=`date -d "$VAL_FECHA_EJECUCION" "+%Y%m01"`
        fi
	echo $f_inicio" Fecha Inicio"
	echo $VAL_FECHA_EJECUCION" Fecha Ejecucion"
...
-fecha_ejecucion $VAL_FECHA_EJECUCION -fecha_inicio $f_inicio -nombre_proceso_pyspark $VAL_NOMBRE_PROCESO_HIVE_QUERY &> $VAL_LOG_EJECUCION_PYTHON
...
# sh -x /home/nae108834/Reingenieria_Cliente360/Bin/OTC_T_360_PARQUE_TRAFICADOR.sh 20230105 0
