    ...
    qry = '''
        SELECT DISTINCT
            ...
            from(
            	...
            	case when (fecha_baja is null or fecha_baja = '') then '2022-11-01 12:21:31.393' else fecha_baja end as fecha_baja_new,
            	estado_abonado,
            	...
            	telefono_cliente_pr,
            	imei,
            	row_number() over (partition by num_telefonico order by (case when (fecha_baja is null or fecha_baja = '') then '2022-11-01 12:21:31.393' else fecha_baja end) desc,fecha_alta desc,nvl(fecha_modif,fecha_alta) desc) as orden
